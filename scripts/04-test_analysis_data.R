#### Preamble ####
# Purpose: Tests the structure and validity of the PollinateTO Primary Project Garden dataset
...
if (all(analysis_data$garden_type %in% valid_gardentype)) {
...
if (all(analysis_data$ward_name %in% valid_wards)) {
...
if (all(is.character(analysis_data$garden_type) 
        & is.character(analysis_data$nia_or_en) 
        & is.character(analysis_data$ward_name))) {
...
if (all(analysis_data$garden_type != "" & 
        analysis_data$nia_or_en != "" & analysis_data$ward_name != "") == TRUE) {
...
if (n_distinct(analysis_data$garden_type) >= 2) {
...
if (n_distinct(analysis_data$ward_name) >= 2) {
...
if (all(analysis_data$year_funded %in% 2020:2023)) {
...
if (all(is.numeric(analysis_data$year_funded) & is.numeric(analysis_data$number_of_gardens) 
        & is.numeric(analysis_data$estimated_garden_size) 
        & is.numeric(analysis_data$is_indigenous_garden))) {
...
if (all(analysis_data$year_funded >= 0 & analysis_data_simulated$number_of_gardens >= 0 
        & analysis_data$estimated_garden_size >= 0) == TRUE) {
